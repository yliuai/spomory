...
| `forget_all_memory` | `confirm=false` | Physically deletes the entire memory graph in one call — every entity, relation, and archived history version — and writes one audit log entry. `forget_memory` is deliberately one-fact-at-a-time; this is its bulk counterpart, for a clean-slate reset instead of calling `forget_memory` N times. **Two-step by design (added 2026-09-21)**: a call left at the default `confirm=false` deletes nothing — it only reports what a real call would remove. `destructive_hint=true` on this tool is just a hint the MCP spec doesn't require a host to enforce; `confirm` is the actual safeguard, so a host that treats the hint as cosmetic (or an agent auto-approving destructive tools) still can't wipe everything in one shot. Deleting for real needs an explicit second call with `confirm=true` |
